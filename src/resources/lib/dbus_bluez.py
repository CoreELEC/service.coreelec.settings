            ...
            for path, interfaces in objects.items():
                if interfaces.get(INTERFACE_ADAPTER):
                    ...
                    if address == selected_address:
                        log.log(f'using adapter {selected}', log.INFO)
                        return path
...
def power_down_all_except(selected_path):
    """
    Power down all Bluetooth adapters except the selected one.

    Args:
        selected_path (str): The object path of the adapter to keep powered on,
                             e.g. '/org/bluez/hci0'
    """
    managed = get_managed_objects()

    for path, interfaces in managed.items():
        if INTERFACE_ADAPTER in interfaces:
            if path == selected_path:
                # Keep this one powered (or ensure it is on)
                if not adapter_get_powered(path):
                    try:
                        adapter_set_powered(path, True)
                        log.log(f"Selected adapter powered up: {path}", log.INFO)
                    except Exception as e:
                        log.log(f"Failed to power up adapter {path}: {e}", log.ERROR)
                continue

            # Power down all others
            if adapter_get_powered(path):
                try:
                    adapter_set_powered(path, False)
                    log.log(f"Adapter powered down: {path}", log.INFO)
                except Exception as e:
                    log.log(f"Failed to power down adapter {path}: {e}", log.ERROR)
